use srvcs_nthroot::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-nthroot listening");
    let deps = Deps {
        root_url: cfg.root_url.clone(),
    };
